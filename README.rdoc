...
4) Run the following rake task to post to Mechanical Turk :
    ...
    # The duration in hours the HIT is available to a worker (e.g. 1 hour)

    rake turkee:post_hit[<Host>, <Title>, <Description>, <Model>, <Number of Assignments>, <Reward>, <Lifetime>, <Duration>]
...
8) When all specified assignments for a HIT have been completed, Turkee will attempt to call the optional hit_complete class method for the model.  This can be used for any cleanup logic.
	class Survey < ActiveRecord::Base
